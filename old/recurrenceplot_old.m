...
% if minlength>0      
%     tempdiag=0;
%     pattern=['01{1,' num2str(N) '}0'];
% tempsumdist=horzcat(zeros(N,1),results.sumdist,zeros(N,1));
% tempsumdist=vertcat(zeros(1,N+2),tempsumdist,zeros(1,N+2));
% for diagindex=0:N-1
%     tempdiag=vertcat(tempdiag,diag(tempsumdist,-diagindex));
% end
% tempdiag(1)=[];
%     [s1,e1]=regexp((num2str(tempdiag))',pattern);
%     t1=(e1-s1)-1;
% det=sum(t1(t1>minlength))/sum(t1);
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if singlenei==1
    temp2([endpos])=-1;
    temp2=vertcat(-1,temp2);
    temp2(end)=[];
    temp2(isnan(temp2)==1)=[];
    ind=find(temp2>0);
    ind2=find(temp2(ind) & temp2(ind-1)==-1);
    temp2=temp2(ind(ind2));
end

if  maxRecPD==0
    maxRecPD=length(results.sumdist);
    minRecPD=1;
end
temp2(isnan(temp2)==1)=[];
results.ht=hist(temp2,1:length(results.sumdist));                     
